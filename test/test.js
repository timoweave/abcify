...
const {is_undefined_literal, compare_name, sort_ast, compare_by_object} = utils;
...
describe("is_undefined_literal", function() {
  it("should be able to handle right types", function() {    
    assert.equal(is_undef({type: GENERIC, id: {name: UNDEFINED, type: IDENTIFIER}}), true);
  });
  ...
  it("should be able to handle {}", function() {
    ...
    assert.equal(is_undef({type: true, id: {name: "hello", type: false}}), false);
    assert.equal(is_undef({type: GENERIC, id: {name: "hello", type: false}}), false);
    assert.equal(is_undef({type: GENERIC, id: {name: "hello", type: IDENTIFIER}}), false);
    assert.equal(is_undef({type: GENERIC, id: {name: null, type: IDENTIFIER}}), false);
    assert.equal(is_undef({type: GENERIC, id: {name: undefined, type: IDENTIFIER}}), false);
  });
...
describe("sort destructuring object by name", function() {
    ...
    const should_handle_value = "should sort object by key, with value --> " +
          "{a: {x: false, y: null, z: 'hello'}, b: 1, c: true}";
    it(should_handle_value, function() {
        const input = "const {c: true, b: 1, a: {z: 'hello', y: null, x: false}} = {a: {}}";
        const output = "const {a: {x: false, y: null, z: 'hello'}, b: 1, c: true} = {a: {}}";
        assert.equal(sort_destructuring_by_key(input), output);
    });
    
    const should_handle_duplicates = "should sort object by key, keep dup order ---> " +
          "{a: 1, a: 2, b: {k: 'hello', k: null}, c: true, c: false}";
    it(should_handle_duplicates, function() {
        const input = "const {c: true, c: false, b: {k: 'hello', k: null}, a: 1, a: 2} = {b: {}}";
        const output = "const {a: 1, a: 2, b: {k: 'hello', k: null}, c: true, c: false} = {b: {}}";
        assert.equal(sort_destructuring_by_key(input), output);        
    });
